...
inline arma::vec generate_bootstrap_dat(const arma::vec& dat,
  ...
  arma::vec y = (x + h_crit * arma::randn(dat.n_elem)) / sqrt(1 + (h_crit * h_crit)/sigma_sq);
  return y;
}
...
inline double bootstrap_test_unimodality(const arma::vec obs_dat, 
  ...
  double p_val = n_greater_1 / (double)n_boot;
  return p_val;
}
