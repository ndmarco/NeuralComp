...
inline Rcpp::List calc_chi_squared_IIGPP(const arma::field<arma::vec>& X,
  ...
  for(int i = burnin_num; i < n_MCMC; i++){
    ...
    D_obs(i - burnin_num) = calc_discrepency(X, n, theta.row(i).t(), basis_coef.row(i).t(), basis_funct, time_inhomogeneous);
    D_sim(i - burnin_num) = calc_discrepency(X_sim, n_sim, theta.row(i).t(), basis_coef.row(i).t(), basis_funct_sim, time_inhomogeneous);
    // if((i % 100) == 0){
